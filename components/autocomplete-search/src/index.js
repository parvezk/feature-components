...
function createSuggestionElement({ suggestion, auxilleryData }) {
  const auxilleryString = auxilleryData ? ` - ${auxilleryData}` : "";
  const boldProcessedSuggestion = wrapBoldedCharacters({
    inputValue: searchInput.value,
    suggestion
  });
  return `<li class="search__suggestions__list__result">${boldProcessedSuggestion}${auxilleryString}</li>`;
}
...
function onSuggestionsResponse(data) {
  ...
  for (const suggestion of data) {
    suggestionsHTML += createSuggestionElement({
      ...
      auxilleryData: suggestion.auxillery
    });
  }
